...
import { framesDue } from "./framesDue";
...
/** A video at 10 frames a second, so frame numbers read as tenths of a second: frame 1012 is at 101.2 s. */
const video = { frameRate: { numerator: 10, denominator: 1 }, durationFrames: 2000 };
...
describe("which frame is due", () => {
  ...
  test("the frame due follows the moment heard, and the frames ahead jump at a Join as the sound does", () => {
    ...
    expect(framesDue(played, 0, video, 0).due).toBe(1000);
    expect(framesDue(played, 0.25, video, 0).due).toBe(1002);
    expect(framesDue(played, 0.55, video, 0).due).toBe(1012);
    // From 0.35 s played, the next 0.45 s show 100.35 and 100.45 s, then after the Join 101.25, 101.35 and 101.45 s.
    expect(framesDue(played, 0.35, video, 0.45)).toEqual({ due: 1003, ahead: [1003, 1004, 1012, 1013, 1014] });
  });
  ...
  test("before the sound starts it is the first frame, and nothing lies past the Recording's last frame", () => {
    ...
    expect(framesDue(played, -0.2, video, 0).due).toBe(1000);
    // The Recording ends at 102.5 s, frame 1024 its last.
    expect(framesDue(played, 2.9, { ...video, durationFrames: 1025 }, 0.3)).toEqual({ due: 1024, ahead: [1024] });
  });
});
